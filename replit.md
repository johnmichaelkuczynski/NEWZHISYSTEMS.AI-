...
- **Authentication (Google-only Clerk)**: Dedicated Clerk app "ZHISYSTEMS.AI" (instance robust-tortoise-29). Login is Google-ONLY via a custom `GoogleSignInButton` using `authenticateWithRedirect` (oauth_google) — no Clerk hosted components, so no email/password fields ever render. Two-click flow: public landing at `/` shows a single Google button; OAuth completes at `/sign-in/sso-callback`. All routes except `/`, `/sign-in*`, `/sign-up*` are gated client-side (redirect to `/`). Server uses `clerkMiddleware()` cookie auth with `requireAuth` on all data/AI endpoints (journal writes, office, higher-ed, all `/api/ai/*`) and `requireAdmin` (johnmichaelkuczynski@gmail.com only) on office CRUD and admin APIs. Keys in CLERK_SECRET_KEY / VITE_CLERK_PUBLISHABLE_KEY; managed Clerk swaps to production (pk_live/sk_live) automatically at publish.
