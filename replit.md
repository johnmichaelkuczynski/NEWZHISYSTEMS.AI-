...
- **Core Applications**: 31 applications organized into 6 categories (Writing & Books, Education & Learning, Intelligence & Psychology, Visual & Multimedia, Audio & Interactive Media, Utility & Conversion).
- **Visitor Analytics ("Administrative" page)**: Client-side tracking posts each route change to `/api/track`; server records IP, user-agent, referer, and geography (geoip-lite) into the `site_visits` table. The `/administrative` page (discrete gray link in NavBar, password "1234" client-side gate) shows visits/unique totals for 24h/7d/30d/all-time, hourly and daily charts (recharts), country/city breakdowns, top pages, and the 200 most recent visits.
...
- **Authentication: NONE (removed by owner's order 2026-07-04)**: The entire login system (Clerk, then direct Google OAuth) was ripped out at the owner's explicit direction — DO NOT REINSTALL any login/auth without his explicit request. The site is fully public. All auth code, session middleware, login UI, the `/administrative` page, and `/api/admin/*` routes were deleted; auth packages uninstalled. The `PasswordGate` password screens were also removed on the owner's follow-up order ("RIP OUT ANY LOGIN"). Exceptions: on 2026-07-06 the owner explicitly requested a password gate (password "1234", client-side, sessionStorage) on the "AI in Higher Ed" page; on 2026-07-13 he requested the same gate (password "1234") on the "/administrative" visitor-analytics page. Every other page and all APIs remain open. Legacy `users`/`visits`/`session` tables may still exist in the DB but are unused and removed from the schema.
