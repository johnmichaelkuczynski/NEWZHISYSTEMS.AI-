...
**How to apply:** All API endpoints are intentionally public (including AI endpoints that spend API credits). The owner later escalated ("RIP OUT ANY LOGIN THAT IS ON THIS APP") and the PasswordGate password screens were removed too. Do not add any gate, password prompt, or auth without an explicit new request. Exception granted 2026-07-06: owner explicitly asked for a client-side password gate ("1234") on the AI in Higher Ed page only. Unused legacy DB tables (users/visits/session) and GOOGLE_* / SESSION_SECRET secrets may remain in the environment.
